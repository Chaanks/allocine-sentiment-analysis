...
def dataprep(df):
    # split dataframe
    df_feats = df.copy().drop(columns=["directors", 'kinds', "review_id", "note_mean", "note_std"]) # "rating", "review" "subscriber", "movie_id", "note_mean", "note_std"
...
if __name__ == "__main__":
    ...
    dtype = {"note": np.float32, "movie_id": str, "rating": np.float32, "review": np.float32, "subscriber": np.float32}
    ...
    model = create_model(feats_encoder, inputs, reviews_encoder, max_features, embedding_dim)
    model.compile(loss="categorical_crossentropy", optimizer="nadam", metrics="accuracy")

    model.fit(
        x=[train_feats_dict, train_reviews], y=train_labels,
    ...
    test_feats = df_test.copy().drop(columns=["directors", 'kinds', "note_mean", "note_std"]) # "rating", "review",	"subscriber", "movie_id", "note_mean", "note_std"
